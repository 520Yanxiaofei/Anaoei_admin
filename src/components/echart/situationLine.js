...
const SituationLine = React.createClass({
	...
	showChart: function(dataSet) {
		...
		let myChart = echarts.init(document.getElementById('SitLineMain'));
		if (loading) {
			myChart.showLoading({
				text: '数据获取中',
				color: '#08beff',
	...
	render: function() {
		return (
			<div id="SitLineMain" style={{ height:300}}></div>
		)
	}
})
