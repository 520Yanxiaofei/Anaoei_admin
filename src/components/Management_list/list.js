		...
		<div>
			 {
			   	alarmlist.map((data,index)=>{
			   		return(
				   		<li key={index} className={styles.warnMassage}>
		  					<Icon type="close-circle"/>
		  					<span>{data.warn_summarize}</span>
		  				</li>
			   		)
			   	})	
	  		}
           	...
           	return  (
				...
				<Col sm={16} md={12} lg={8} className={styles.feicard}>
					   <div  key={index} className={styles.feicardAnimate}>
				        {
				        data.host_status==3?
					    <div className={styles.ManageDelBtn} onClick={()=>ManageDel(data.host_id,data.host_name,index)}><Icon type="close-circle-o" /></div>
					    :null
					    }
